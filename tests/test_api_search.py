...
class TestApiSearchClass(unittest.TestCase):
    ...
    def test_create_update_from(self):
        ...
        """
        desired_output = "/var/yang/all_modules/yang-catalog@2018-04-03.yang:1: error: the grouping 'yang-lib-imlementation-leafs', "\
            "defined at /var/yang/all_modules/yang-catalog@2017-09-26.yang:599 is illegally removed\n"
        file1 = 'yang-catalog'
        revision1 = '2018-04-03'
        file2 = 'yang-catalog'
        revision2 = '2017-09-26'

        result = search_bp.create_update_from(file1, revision1, file2, revision2)
        response_text = ET.fromstring(result).find('body').find('pre').text
    ...
    def test_create_update_from_raise_exception(self, mock_os_makedirs: mock.MagicMock):
        ...
        self.assertEqual(result, 'Server error - could not create directory')
    ...
    def test_search_vendor_statistics(self):
        ...
        result = search_bp.search_vendor_statistics(vendor_name)
        payload = json.loads(result.data)
    ...
    def test_search_vendors(self):
        ...
        result = search_bp.search_vendors(path)
        payload = json.loads(result.data)
    ...
    def test_search_vendors_vendor_not_found(self):
        ...
        with self.assertRaises(NotFound) as http_error:
            search_bp.search_vendors(path)

        self.assertEqual(http_error.exception.code, 404)
        self.assertEqual(http_error.exception.description, 'No vendors found on path vendor={}'.format(vendor_name))
    ...
    def test_search_module(self):
        ...
        revision = '2018-04-03'
        organization = 'ietf'
        result = search_bp.search_module(name, revision, organization)
        payload = json.loads(result.data)
    ...
    def test_get_modules_no_modules(self, mock_redis_get: mock.MagicMock):
        ...
        mock_redis_get.return_value = None
        with self.assertRaises(NotFound) as http_error:
            search_bp.get_modules()

        self.assertEqual(http_error.exception.code, 404)
        self.assertEqual(http_error.exception.description, 'No module is loaded')
        self.assertEqual(http_error.exception.name, 'Not Found')

    def test_get_vendors(self):
        """Test if vendors json payload has correct form (should not contain empty 'vendor' list)
        """
        result = search_bp.get_vendors()
        payload = json.loads(result.data)
    ...
    def test_get_vendors_no_vendors(self, mock_redis_get: mock.MagicMock):
        ...
        mock_redis_get.return_value = None
        with self.assertRaises(NotFound) as http_error:
            search_bp.get_vendors()

        self.assertEqual(http_error.exception.code, 404)
        self.assertEqual(http_error.exception.description, 'No vendor is loaded')
        self.assertEqual(http_error.exception.name, 'Not Found')

    def test_get_catalog(self):
        """Test if catalog json payload has correct form (should contain both 'vendors' and 'modules' data)
        """
        result = search_bp.get_catalog()
        payload = json.loads(result.data)
    ...
    def test_get_catalog_no_catalog_data(self, mock_redis_get: mock.MagicMock):
        ...
        mock_redis_get.return_value = None
        with self.assertRaises(NotFound) as http_error:
            search_bp.get_catalog()

        self.assertEqual(http_error.exception.code, 404)
        self.assertEqual(http_error.exception.description, 'No data loaded to YangCatalog')
        self.assertEqual(http_error.exception.name, 'Not Found')
    ...
    def test_create_tree(self):
        ...
        result = search_bp.create_tree(filename, revision)
        response_text = ET.fromstring(result).find('body').find('pre').text
    ...
    def test_create_reference(self):
        ...
        result = search_bp.create_reference(filename, revision)
        response_text = result.split('</pre>')[0].split('<pre>')[1]

        with open('{}yang-catalog@2018-04-03.yang'.format(self.resources_path), 'r') as f:

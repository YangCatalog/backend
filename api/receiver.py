...
class Receiver:
    ...
    def process_vendor_deletion(self, arguments: list):
        ...
        self.iterate_in_depth(vendor_data, modules)
        ...
        for mod in modules:
            try:
                ...
                self.LOGGER.exception('YANG file {} doesn\'t exist although it should exist'.format(mod))
                ...
                if existing_module.get('dependents') is not None:
                    ...
                    for dep in dependents:
                        if dep['name'] == name and dep['revision'] == rev:
                            ...
                            if response.status_code != 204:
                                self.LOGGER.error('Could not delete module {}. Error: {}'.format(mod_key, response.text))
                                return '{}#split#{}'.format(self.__response_type[0], response.text)

        # Delete vendor data itself
        response = requests.delete(path_to_delete, auth=(credentials[0], credentials[1]))
        if response.status_code == 404:
            self.LOGGER.info('Path {} already deleted'.format(path_to_delete))
    ...
    def iterate_in_depth(self, value: dict, modules: set):
        """Iterates through the branch to get to the level with modules.

        Arguments:
            :param value    (dict) data through which we will need to iterate
            :param modules  (set) set that will contain all the modules that need to be deleted
        """
